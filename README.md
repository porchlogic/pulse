...
The frontend is currently a lightweight multi-view app that loads route templates into a single shell. Runtime styling is loaded from `./assets/styles/unified.css`.

## Legacy shared-css sync

`scripts/sync-shared-assets.sh` is now guarded and will skip by default, because the app no longer references `foundation.css`.
It only performs a copy if `index.html` explicitly references `assets/styles/foundation.css`.

## Versioning for cache busting

The app router appends `?v=<version>` to template/module URLs. That version comes from `version.json` and uses semantic version format (for example `0.1.2`).

To ensure every deploy invalidates browser caches, bump `version.json` during deploy:

```bash
bash web/pulse.porchlogic.com/scripts/update-version.sh
```
...
In CI/CD, run `update-version.sh` on every push/release before publishing `web/pulse.porchlogic.com`.
