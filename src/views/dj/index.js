...
export const mount = ({ mountNode, navigate }) => {
    ...
    const accountForm = mountNode.querySelector("[data-role=\"dj-account-form\"]");
    const currentPasswordInput = mountNode.querySelector("#dj-current-password");
    const newUsernameInput = mountNode.querySelector("#dj-new-username");
    const newPasswordInput = mountNode.querySelector("#dj-new-password");
    const accountMessageEl = mountNode.querySelector("[data-role=\"dj-account-message\"]");
    const accountSubmitButton = mountNode.querySelector("[data-role=\"dj-account-submit\"]");
    ...
    const renderIdentity = () => {
        const username = latestState.auth.user?.username || latestState.auth.user?.email || "unknown";
        identityEl.textContent = `logged in as: ${username}`;
    ...
    const onAccountSubmit = async (event) => {
        ...
        if (nextUsername) {
            payload.newUsername = nextUsername;
        }
        if (nextPassword) {
            payload.newPassword = nextPassword;
        }

        const result = await djChangeCredentials(payload);
        accountSubmitButton.disabled = false;
        if (!result.ok) {
            accountMessageEl.textContent = result.error || "Failed to update credentials";
        ...
        accountMessageEl.textContent = "Credentials updated";
        accountMessageEl.hidden = false;
    };
    ...
    return () => {
        ...
        accountForm.removeEventListener("submit", onAccountSubmit);
    };
};
