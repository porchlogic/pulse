...
import { persistNoiseFloorEnabled, readStoredNoiseFloorEnabled } from "./components/audio/noise-floor.js";
...
const normalizeBarsPerRow = (value) => {
...
const initialNoiseFloorEnabled = readStoredNoiseFloorEnabled();
const state = {
    pulse: {
        ...
        noiseFloorEnabled: initialNoiseFloorEnabled,
    ...
    performance: {
        ...
        liveDjUserId: "",
        bpm: 120,
        barsPerRow: DEFAULT_BARS_PER_ROW,
        tickLength: DEFAULT_TICK_LENGTH,
    ...
    troupe: {
        trackSelectionCounts: Array.from({ length: TRACK_COUNT }, () => 0),
    },
};
...
const applyPulseSnapshot = (pulse) => {
    if (!pulse || typeof pulse !== "object") {
        return;
    }
    patchPulseState(pulse);
};
        ...
        onRealtimeEvent: (message) => {
            if (message.type === "performance_state") {
                ...
                state.performance = {
                    ...
                    barsPerRow: normalizeBarsPerRow(message.barsPerRow),
                    tickLength: Number(message.tickLength || state.performance.tickLength),
                    themeColors: normalizeThemeColors(message.themeColors || state.performance.themeColors),
                    updatedAt: message.updatedAt || "",
                };
                notify();
            }
            if (message.type === "troupe_track_counts") {
                const counts = Array.isArray(message.counts)
                    ? Array.from({ length: TRACK_COUNT }, (_, index) => {
                        const value = Math.trunc(Number(message.counts[index] || 0));
                        return Number.isFinite(value) ? Math.max(0, value) : 0;
                    })
                    : Array.from({ length: TRACK_COUNT }, () => 0);
                patchTroupeState({ trackSelectionCounts: counts });
            }
            if (message.type === "dj_perform_action") {
                debugLog("state", "dj_perform_action received", message);
                const previousSongId = String(state.performance.lastAction?.songId || "");
                const nextSongId = String(message.songId || "");
                const hasThemeColors = Array.isArray(message.themeColors) && message.themeColors.length > 0;
                const applyThemeFromSongChange = message.action === "play" && Boolean(nextSongId) && nextSongId !== previousSongId;
                const applyThemeFromThemeAction = message.action === "theme";
                const shouldApplyTheme = hasThemeColors && (applyThemeFromSongChange || applyThemeFromThemeAction);
                state.performance = {
                    ...state.performance,
                    barsPerRow: normalizeBarsPerRow(message.barsPerRow || state.performance.barsPerRow),
                    themeColors: shouldApplyTheme
                        ? normalizeThemeColors(message.themeColors)
                        : state.performance.themeColors,
                    lastAction: message,
                };
                notify();
            }
...
export const setNoiseFloorEnabled = (enabled) => {
    const normalized = Boolean(enabled);
    patchPulseState({
        noiseFloorEnabled: normalized,
    });
    persistNoiseFloorEnabled(normalized);
    return normalized;
};
...
const patchPerformanceState = (changes) => {
    state.performance = {
        ...state.performance,
        ...changes,
        barsPerRow: normalizeBarsPerRow(changes?.barsPerRow ?? state.performance.barsPerRow),
        themeColors: normalizeThemeColors(changes?.themeColors ?? state.performance.themeColors),
    };
    notify();
};
...
const patchTroupeState = (changes) => {
    state.troupe = {
        ...state.troupe,
        ...changes,
    };
...
const normalizeSong = (song, fallbackIndex = 0) => ({
    ...
    themeColors: normalizeThemeColors(song?.themeColors),
...
export const loginDj = async ({ username, password }) => {
    patchAuthState({ loading: true, error: "" });
    try {
...
export const loginMember = async ({ username, password }) => loginDj({ username, password });
...
export const canAccessRoute = (route) => {
    ...
    if (normalizedRoute === "troupe") {
        return isAuthenticated && (role === "troupe" || isArtistRole(role) || role === "director");
    }
    if (normalizedRoute === "dj" || normalizedRoute === "perform") {
        return isAuthenticated && isArtistRole(role);
    }
    if (normalizedRoute === "director") {
        return isAuthenticated && role === "director";
    }
    return true;
};
...
export const sendTroupeTrackSelection = (trackSelection) => {
    const normalizeTrackIndex = (value) => Math.max(0, Math.min(TRACK_COUNT - 1, Math.trunc(Number(value) || 0)));
    const selectedTrackIndexes = Array.isArray(trackSelection)
        ? [...new Set(trackSelection.map(normalizeTrackIndex))]
        : [normalizeTrackIndex(trackSelection)];
    getTransportClient().sendRealtime({
        type: "troupe_track_selection",
        selectedTrackIndex: selectedTrackIndexes[0] ?? null,
...
export const directorListAccounts = async () => {
    try {
        const payload = await apiClient.request("/pulse/director/accounts");
        ...
        patchDirectorState({ error: error.message || "Failed to load accounts" });
        return { ok: false, error: error.message || "Failed to load accounts" };
    }
};

export const directorCreateAccount = async (payload) => {
    try {
        const response = await apiClient.request("/pulse/director/accounts", {
            method: "POST",
            body: payload,
        });
        ...
        if (account) {
            patchDirectorState({
                accounts: [...state.director.accounts, account],
                error: "",
            });
        }
        return { ok: true, account };
    } catch (error) {
        patchDirectorState({ error: error.message || "Failed to create account" });
        return { ok: false, error: error.message || "Failed to create account" };
    }
};

export const directorUpdateAccount = async (userId, payload) => {
    try {
        const response = await apiClient.request(`/pulse/director/accounts/${encodeURIComponent(userId)}`, {
            method: "PATCH",
            body: payload,
        });
        const account = response?.account || null;
        if (account) {
            patchDirectorState({
                accounts: state.director.accounts.map((item) => (item.id === userId ? account : item)),
                djs: state.director.djs.map((item) => (item.id === userId
                    ? { ...item, username: account.username, displayName: account.displayName }
                    : item)),
                error: "",
            });
        }
        ...
        patchDirectorState({ error: error.message || "Failed to update account" });
        return { ok: false, error: error.message || "Failed to update account" };
    }
};

export const directorDeleteAccount = async (userId) => {
    try {
        const response = await apiClient.request(`/pulse/director/accounts/${encodeURIComponent(userId)}`, {
            method: "DELETE",
        });
        ...
        if (account) {
            patchDirectorState({
                accounts: state.director.accounts.map((item) => (item.id === userId ? account : item)),
                djs: state.director.djs.filter((item) => item.id !== userId),
                error: "",
            });
        }
        return { ok: true, account };
    } catch (error) {
        patchDirectorState({ error: error.message || "Failed to delete account" });
        return { ok: false, error: error.message || "Failed to delete account" };
    }
};

export const djChangeCredentials = async ({ currentPassword, newUsername, newPassword }) => {
    try {
        const payload = await apiClient.request("/auth/change-credentials", {
            method: "POST",
            body: {
                currentPassword,
                ...(typeof newUsername === "string" ? { newUsername } : {}),
        ...
        return { ok: false, error: error.message || "Failed to update credentials" };
    }
};

export const createTroupeAccount = async ({ username, password, email }) => {
    try {
        const payload = await apiClient.request("/auth/create-troupe-account", {
            method: "POST",
        ...
        return { ok: false, error: error.message || "Failed to create troupe account" };
    }
};
...
export const djSetBarsPerRow = async (barsPerRow) => {
    const payload = await apiClient.request("/pulse/dj/bars-per-row", {
        method: "POST",
        body: { barsPerRow },
    });
    if (payload?.performance) {
